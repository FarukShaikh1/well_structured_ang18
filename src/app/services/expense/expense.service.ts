...
export class ExpenseService {
  ...
  adjustExpense(expenseAdjustmentForm: any): Observable<any> {
    return this.http.post(API_URL.EXPENSE_ADJUSTMENT + this.loggedInUserId, expenseAdjustmentForm);
  }

  updateExpense(expenseDetailsForm: any): Observable<any> {
    return this.http.post(API_URL.UPDATE_EXPENSE + this.loggedInUserId, expenseDetailsForm);
  }

  deleteExpense(expenseId: string): Observable<any> {
    return this.http.get(API_URL.DELETE_EXPENSE + expenseId + '&userId=' + Number(localStorage.getItem("userId")));
  }
