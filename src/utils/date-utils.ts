...
export class DateUtils {
  ...
  static checkDateFilter(occasionDateString: string, isToday: boolean, isYesterday: boolean, isTomorrow: boolean): boolean {
    if (!occasionDateString) return false;

    const occasionDate = new Date(occasionDateString);

    // Extract only day and month (ignore year)
    const occDay = occasionDate.getDate();
    const occMonth = occasionDate.getMonth(); // 0–11
    ...
    const isTodayMatch =
      isToday &&
      occDay === today.getDate() &&
      occMonth === today.getMonth();

    const isYesterdayMatch =
      isYesterday &&
      occDay === yDay.getDate() &&
      occMonth === yDay.getMonth();

    const isTomorrowMatch =
      isTomorrow &&
      occDay === tom.getDate() &&
      occMonth === tom.getMonth();

    // If no filter selected → allow all
    if (!isToday && !isYesterday && !isTomorrow) return true;
